...
```bash
cd vendor/s-ui
cp run.env.example run.env   # SUI_RUN_HOST, при необходимости SUI_RUN_CERTBOT_EMAIL
...
4. Запуск панели и фонового renew:

   ```bash
   docker compose -f docker-compose.stand.yml up -d
   ```

Сервис `certbot-renew` раз в 12 часов вызывает `certbot renew`; при успешном продлении отправляется **HUP** контейнеру `s-ui-local`, процесс перезапускает приложение и подхватывает новые файлы (симлинки в `/etc/letsencrypt/live/...`).
