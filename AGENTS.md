...
- **Дата:** 2026-05-07  
- **Старт итерации / текущий фокус:** CONNECT-IP **UDP-мост (клиент)** и **серверный `ReadPacket`**: переиспользование буфера и шаблон IPv4+UDP заголовка (`As4`), сериализация записей под `writeMu`, обновление `maxPayload` внутри цикла фрагментации после PTB/recovery (исправление логической ошибки предыдущей версии цикла), счётчики bridge write path + тик `maybeEmitConnectIPActiveSnapshot` на egress для дельт раннера; на сервере **`connectIPNetPacketConn.ReadPacket`** — **`Advance`/`Truncate`** вместо memmove payload. Прежний патч QUIC H3 DATAGRAM (`EnqueuePooledHTTPDatagram` / **`OutgoingPayloadRelease`**) остаётся базой для исходящего DATAGRAM. **Compose-матрица скорости в этой сессии не переснималась** (Docker Desktop / паритет CI — Linux/WSL).
- **`hiddify-core` `HEAD`:** **`6e823047b7a3fce281cacc770c64f40f2dddc0c4`**.
...
4. **`hiddify-app`:** обновить указатель submodule на **`hiddify-core`** `6e823047…` после push submodule (локально коммит уже есть).
