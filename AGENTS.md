...
- **Матрица скорости (обязательно обновлять в каждой итерации):**
  ...
  2. **Историческая PASS-точка:** **`140000000`** (старый артефакт: `bytes_received=20971520`, `loss_pct=0.0`, hash OK).
  3. **Предыдущий FAIL-прогон на этой машине (до правки tun0 guard):** `MASQUE_STAND_SLOW_DOCKER=1`, `MASQUE_TCP_IP_SOCKET_BUF_BYTES=134217728`, `python masque_stand_runner.py --scenario tcp_ip --megabytes 20 --tcp-ip-mode bulk_single_flow --udp-send-bps 140000000` → `bytes_received=20690240`, `loss_bytes=281280`, `loss_pct=1.3412`, `hash_ok=false`, `receiver_settled=false`, `budget_margin_sec=-0.187`, `stop_reason=bridge_boundary_stall`, `route_on_tun0=true`.
  4. **Свежий прогон `tcp_ip` 20 MiB @ 140M на этой машине (до хуков server OBS):** `MASQUE_STAND_SLOW_DOCKER=1`, `MASQUE_TCP_IP_SOCKET_BUF_BYTES=134217728`, `python masque_stand_runner.py --scenario tcp_ip --megabytes 20 --tcp-ip-mode bulk_single_flow --udp-send-bps 140000000` → `bytes_received=20937532`, `loss_bytes=33988`, `loss_pct=0.1621`, `hash_ok=false`, `receiver_settled=false`, `budget_margin_sec=-0.106`, `stop_reason=near_full_micro_loss_at_deadline`, `ok=false`; при этом `route_on_tun0=true`, `tun0_dev_bytes.delta.tx_bytes=21472840`, `tun0_dev_bytes.delta.rx_bytes=0`, `observability_peer_split=true`, `delta_client>0`, `delta_server=0` (ожидаемо исправлено зеркалированием счётчиков на **`connectIPNetPacketConn`** — см. §6 и коммит в **`hiddify-core`**).
...
- **Старт итерации / текущий фокус:** 
  - В **`hiddify-core`**: серверный **`connectIPNetPacketConn`** (`endpoint_server.go`) теперь вызывает **`ObserveConnectIPServerReadError` / `ObserveConnectIPServerReadSuccess` / `ObserveConnectIPServerWriteIteration`** из **`transport/masque`**, чтобы **`CONNECT_IP_OBS`** и **`connectIPCounters`** на процессе **`masque-server-core`** отражали реальный CONNECT-IP packet-plane (раньше считались только на клиентском **`connectIPPacketSession`**).
  - `go test -count=1 -short -tags with_masque ./protocol/masque/... ./transport/masque/...` (cwd **`hiddify-core/hiddify-sing-box`**) — **PASS**; Python контрактные тесты раннера — **63 PASS**.
  - Строгий Docker `tcp_ip` @ 140M на этом хосте после коммита: перезаписать строку **4** матрицы по свежему **`runtime/*.json`** (`delta_server`, `connect_ip_packet_rx_total` на сервере).
- **Route-guard (typed fail-fast):** контракт `route_guard_target_not_on_tun0` сохранён; диагностика `tun0_dev_bytes` без ослабления PASS-критериев.
- **`hiddify-core` `HEAD`:** **`60c9ebe23103a455f9f75df786e312e704990dfd`** (server **`connectIPNetPacketConn`** → **`ObserveConnectIPServer*`**).
- **Лестница:** `max_pass=140000000` — после появления непустого **`delta_server`** снова оценить near-full micro-loss (возможный хвост sink/тайминг vs реальные дропы).
- **Контрольные прогоны после правок:** `go test` (см. выше) + `python -m unittest test_masque_runtime_contract_validator test_masque_runtime_ci_gate_asserts test_masque_stand_runner_smoke_contract` — **PASS**; полный **`tcp_ip` strict @ 140M** — повторить локально после коммита/образа.
...
1. **Стабилизация 140M:** 2× strict Docker-прогона с фиксацией `loss_bytes`, `budget_margin_sec`, `delta_server` (**ожидается `delta_server>0`** после хуков), `tun0_dev_bytes.delta`, **`connect_ip_packet_rx_total`** в server-логах.
2. **Если micro-loss сохранится при непустом `delta_server`:** triage хвоста приёмника / near-full window (без ослабления hash/ratio), при необходимости QUIC/datagram sink — по **`AGENT-LAYER-SOURCE-OF-TRUTH.md`**.
3. **Анти-bypass negative control:** локально `route_on_tun0=false` → `stop_reason=route_guard_target_not_on_tun0`, `error_class=policy`, `tun0_dev_bytes.activity=false`.
4. **Submodule:** коммит в **`hiddify-core`** + обновление SHA в **`hiddify-app`**.
