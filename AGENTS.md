...
|------|----------|
| Дата | 2026-05-10 |
| Фокус | H2 dataplane MASQUE; автономный аудит кода эндпоинта/транспорта (без расширения документации ради синхронизации). |

**Чеклист текущей итерации:** нет открытых задач.

**Последняя правка кода / аудит этой итерации:** Автономный проход по **`transport/masque`**, **`protocol/masque`**, **`common/masque`**, форкам **`third_party/connect-ip-go`**, **`third_party/masque-go`**: **`go test`** (перечисленные пакеты + модули форков) — OK; **`go vet`** — OK; **`staticcheck`** на masque-пакеты — OK. Точечный обзор цепочек **`currentUDPHTTPLayer`**, **`dialTCPStream*` / H2 Extended CONNECT**, **`dialUDPOverHTTP2`**, **`dialConnectIPHTTP2`**, **`IsMasqueHTTPLayerSwitchableFailure` / `ClassifyMasqueFailure`**, TTL-кэша **`http_layer`** — расхождений и явных дефектов не выявлено. **`go test -race ./transport/masque/...`** на Windows дал **SIG / access violation** в рантайме (типично для связки **race** + in-process **QUIC/http3**; не использован как сигнал дефекта MASQUE; см. осторожность в §15). **Изменений исходного кода в этой итерации не вносилось** — только обновление §13.
