...
**Локально Windows:** `go test -race` — **`CGO_ENABLED=1`**; после cross-build Linux сбросить **`GOOS`/`GOARCH`**. PowerShell: цепочки через **`;`**, не `&&`. WSL bash: использовать **`docker`**, не `docker.exe`. Для **`go test -C …/connect-ip-go`** (как в **`masque-gates`**) флаг **`-C`** должен быть **первым сразу после `go test`** (иначе отказ парсера флагов Go). Harness in-process HTTP/3 proxy в **`transport_test`** после фикса сериализации shutdown проходит **`-race`** на Windows; иной **access violation** — смотреть двойное закрытие **`PacketConn`** / фоновые горутины quic.

## 7) Current autonomous cycle (overwrite each iteration)

- **Дата:** 2026-05-07  
- **Слой / правка:** **`transport/masque/transport.go`** — восстановлены **`RegisterConnectIPServerParseDropSupplier`** и ключ **`connect_ip_server_parse_drop_total`** в **`ConnectIPObservabilitySnapshot`**; в **`protocol/masque`** закоммичен отсутствовавший в индексе **`connect_ip_obs_register.go`** (init → transport).  
- **`hiddify-core`:** **`9c8b7f97`** (fix: связывание server parse-drop с CONNECT_IP_OBS). В родителе **`hiddify-app`** — обновить gitlink submodule при приёмке.  
- **Прогоны в этой сессии (Windows, без Docker):** unit gate **`./protocol/masque ./transport/masque ./common/masque ./include`** — PASS; **`-race`** на **`./protocol/masque ./transport/masque`** (**`CGO_ENABLED=1`**) — PASS; fast regression + fast integration (вкл. **`MASQUE_EXPERIMENTAL_QUIC=1`**, **`connect-ip-go`** с **`-C` первым**) + lifecycle harness — PASS; **`--assert-connect-stream-non-auth-status-matrix`** из **`hiddify-sing-box`** — PASS. Сборка Linux-артефакта, **`masque_stand_runner`**, strict **`tcp_ip`**, anti-bypass, contract validator — **не запускались**.

## 8) Next iteration tasks (single-thread, code-first)

1. **`cd hiddify-core/hiddify-sing-box`:** при необходимости полный повтор шага **`MASQUE unit and race gates`** и остального pre-docker из **`ci.yml`** на целевой ОС; убрать мусор/untracked внутри submodule, если мешает чистому **`git status`**.  
2. **Docker / WSL2:** как в **`masque-gates`**: build **`sing-box-linux-amd64`**, **`masque_stand_runner.py --scenario all`**, **`tcp_ip`** **10/20/50** MiB, **`tcp_ip_scoped`**, затем anti-bypass negatives, **`masque_runtime_contract_validator.py`**, typed asserts (**`--assert-schema`**, **`--assert-anti-bypass-parity-rows`**, **`--assert-scoped-parity-rows`**).  
3. Job **`masque-gates-runtime`** (ночной) — по **`ci.yml`**: **`real` + iperf**, **`tcp_ip --stress`**, **`--assert-nightly-perf-thresholds`** — при регрессии perf смотреть **`hiddify-core/docs/masque-perf-gates.md`**.  
4. При фейле стенда — triage по **`AGENT-LAYER-SOURCE-OF-TRUTH.md`** (**`delta_client`**/**`delta_server`**, **`bridge_boundary_stall`**, **`connect_ip_server_parse_drop_total`** / **`http3_stream_datagram_queue_drop_total`**).
