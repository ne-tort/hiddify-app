...
- **Дата:** 2026-05-07  
- **Старт итерации / текущий фокус:** patched **QUIC/http3:** **`streamDatagramQueueLen` 128 → 1024** в **`hiddify-sing-box/replace/quic-go-patched/http3/state_tracking_stream.go`** — больший буфер между приёмом H3 DATAGRAM и **`ReceiveDatagram`** на CONNECT-IP потоке; цель — снять молчащие дропы **`http3_stream_datagram_queue_drop_total`** на всплесках goodput без смягчения порогов раннера. Прежний near-full **`BULK_TCP_IP_NEAR_FULL_RECV_*`** для хвоста по времени сохранён. **`tcp_ip` compose в этом цикле не гонялся** (нет активного Docker у агента); ниже — последний зафиксированный снимок матрицы **до** верификации патча на стенде.
- **`hiddify-core` `HEAD`:** **`ae64eda0462f12559a845471861408bddd4da6c7`**.
- **Отправная точка (current baseline):** `20 MiB`, `tcp_ip`, `bulk_single_flow`, `--udp-send-bps` (байт/с).
- **Последний зафиксированный матрица-снимок (Docker Desktop строго, до расширения очереди):** **`130000000`** B/s — **PASS** (ноль `loss_bytes` в отмеченном прогоне). **`140000000`** — **FAIL**, `bytes_received≈20793376`, **`loss_pct≈0.85`**, ниже порога near-full (**99.5%**).
- **Лестница с `MASQUE_STAND_SLOW_DOCKER=1`:** **`max_pass=130000000`**, **`next_boundary=140000000`**; после патча очереди переизмерить **140M** и при успехе пробовать **150000000**.
- **Предыдущая PASS-точка (строгий профиль):** **`130000000`** (`--udp-send-bps`).
- **Контрольные прогоны после правок:** **`go test -count=1 -short -tags with_masque`** по **`./transport/masque/... ./protocol/masque/... ./common/masque/...`** — PASS; **`go test -count=1 -short ./http3/...`** с cwd **`hiddify-sing-box/replace/quic-go-patched`** — PASS; **`python -m unittest`** три контрактных модуля **`experiments/router/stand/l3router`** — PASS. **E2e** — ждёт стенд/CI.
...
1. **Стенд/CI после `streamDatagramQueueLen=1024`:** **`tcp_ip`**, **20 MiB**, **`130000000` / `140000000`** (и шаг **`150000000`** если **140M** стал PASS) — записать **`loss_pct`**, **`http3_stream_datagram_queue_drop_total`** / peer-split OBS; подтвердить, была ли связь потерь с per-stream backlog.
2. **`connect_udp` на потолке:** `udp`, `20 MiB`, высокие `--udp-send-bps`; сравнить с CONNECT-IP — общий ли потолок QUIC DATAGRAM.
3. **`wire.DatagramFrame.Append`** (`append(b, f.Data...)`) — только при сигнале профиля (**`sendmsg_ok`** vs фактическая доставка).
4. **Push/sync:** **`hiddify-core`** уже содержит коммит очереди; родитель **`hiddify-app`** — обновить указатель **`hiddify-core`** + при необходимости **`AGENTS.md`** одним или отдельным коммитом от **`experiments/...`**.
