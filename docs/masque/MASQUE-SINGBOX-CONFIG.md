...
| `profile.dataplane_port` | После разрешения host из API: подмена **UDP/QUIC-порта** для MASQUE-дозвона (например `443`, если peer от API указывает на WG-порт). Реализация: [`warp_control_adapter.go`](../../hiddify-core/hiddify-sing-box/protocol/masque/warp_control_adapter.go). |
