...
- **[IN PROGRESS] Стендовая матрица после hot-path правок:** ожидается прогон `degrade_matrix` на образе/`sing-box-linux-amd64`, собранном с **`hiddify-core` `5fefb01e`** (per-stream HTTP/3 DATAGRAM backlog **16k** по умолчанию + `recvClosed` fast-path в `enqueueDatagram`); артефакт `runtime/connect_ip_udp_degrade_matrix.json` — §1 обновлять только после ≥3 повторений или каноничного WSL2+Docker на фиксированном бинарнике с peer-split.
