...
- **[IN PROGRESS] Стендовая матрица после hot-path правок (packer/RLock/OBS):** полный `degrade_matrix` + deltas `http3_*_drop_total`, `quic_datagram_packer_oversize_drop_total`, `connect_ip_packet_write_fail_reason_total["mtu"]` и `last_pass/first_fail` против `connect_ip_udp_degrade_matrix.json`. Уже: `tcp_ip_threshold` — fail в sink/budget при нулевых deltas packer/oversize; быстрый `tcp_ip` @160 Mb/s + `--udp-send-bps 25000000` — PASS без queue drops; после `ac74034` — **нужен новый прогон**.
