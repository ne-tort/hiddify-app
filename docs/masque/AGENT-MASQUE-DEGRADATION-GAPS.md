...
- **[IN PROGRESS] Граница CONNECT-IP ~140 Mb/s (стенд):** на **`hiddify-core` `7d281185`** + Docker Desktop Win прогон **`degrade_matrix`** `130m,140m,150m` дал **last_pass=130m**, **first_fail=140m** (артефакт `runtime/connect_ip_udp_degrade_matrix.json`); отдельный прогон только **130m+140m** — **оба PASS** (дисперсия). Нужны **≥3** повтора или **WSL2** для baseline; счётчики QUIC/HTTP3 drop в peer-split — **0**, симптом — **`sink_writer_boundary_no_udp_errors`**.
