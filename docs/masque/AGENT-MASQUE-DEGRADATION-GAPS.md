...
- **[DONE] HTTP/3 stream DATAGRAM backlog headroom (ядро, не раннер):** в `replace/quic-go-patched/http3/state_tracking_stream.go` увеличен дефолт per-stream очереди до `4096` и добавлен env-тюнинг `HIDDIFY_HTTP3_STREAM_DATAGRAM_QUEUE_LEN` (clamp `128..65536`). Это компенсирующая мера против `http3_stream_datagram_queue_drop_total` при burst.
