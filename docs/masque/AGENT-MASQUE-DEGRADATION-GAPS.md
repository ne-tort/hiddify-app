...
- **[IN PROGRESS] Подтверждение эффектов на стенде:** сравнить до/после по `http3_stream_datagram_queue_drop_total`, `quic_datagram_packer_oversize_drop_total`, `connect_ip_packet_write_fail_reason_total["mtu"]`, а также `max_pass/first_fail` на одинаковой лестнице.
- **[IN PROGRESS] Локальный MTU retry — первичная проверка:** прогон `tcp_ip_threshold` (`120/140/160/180 mbit/s`, 10 MiB, fresh rebuild) показал `delta_quic_packer_oversize=0`, `delta_http3_*_drop=0` на всех шагах; fail остаётся в `sink_writer_boundary_no_udp_errors`/`budget_exceeded`. Вывод: мера корректна как защитная для MTU-событий, но текущий bottleneck, вероятно, вне MTU-path.
...
- **[IN PROGRESS] ACK-vs-DATAGRAM arbitration — стендовая проверка:** проверить влияние на `max_pass/first_fail` и на stop-reason (`budget_exceeded`/sink gap) на одинаковой degrade-лестнице; отдельно зафиксировать deltas `quic_datagram_packer_oversize_drop_total` и `http3_*_drop_total`.
- **[IN PROGRESS] ACK-vs-DATAGRAM arbitration — quick стенд-факт:** `tcp_ip` (10 MiB, `MASQUE_TCP_IP_RATE_LIMIT=160mbit`, `--udp-send-bps 25000000`) даёт `loss_pct=0`, `hash_ok=true`, `stop_reason=none`, при `delta_quic_packer_oversize=0` и `delta_http3_*_drop=0`; требуется полноценная лестница `max_pass + next_boundary` для сравнения с базой.
