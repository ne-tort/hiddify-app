...
- **[IN PROGRESS] Стендовая матрица после hot-path правок:** `degrade_matrix` на Docker Desktop Win с пересборкой `sing-box-linux-amd64` выполняется; `runtime/connect_ip_udp_degrade_matrix.json` обновлён — peer-split дельты (`delta_http3_*`, `delta_quic_*`) часто нулевые при малых потерях и `sink_writer_boundary_no_udp_errors`, а **лимит последнего PASS** по CONNECT‑IP **флапает** между прогонами (см. `AGENTS.md` цикл актуальной даты). **Следующий шаг канона:** прогон на бинарнике с `f78b8248`, ≥3 сэмпла или WSL2, затем обновление §1 против стабильного `last_pass` при согласованных счётчиках.
