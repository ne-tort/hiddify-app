...
- **[IN PROGRESS] Стендовая матрица после hot-path правок:** ожидается прогон `degrade_matrix` на образе/`sing-box-linux-amd64`, собранном с **`hiddify-core` `a6d6334`** (короче критсекция `enqueueDatagram` + pool в CONNECT-UDP `WriteTo`); артефакт `runtime/connect_ip_udp_degrade_matrix.json` при прошлых прогонах: peer-split дельты часто нулевые при `sink_writer_boundary_no_udp_errors`, **last_pass** CONNECT-IP между сэмплами флапает — §1 актуализировать только после ≥3 повторений или каноничного WSL2+Docker на фиксированном бинарнике.
