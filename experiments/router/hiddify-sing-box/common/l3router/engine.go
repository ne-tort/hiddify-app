...
func (s *memEngineState) clone() *memEngineState {
	next := &memEngineState{
		...
		ingressExists: append([]bool(nil), s.ingressExists...),
		lookup:        s.lookup,
	}
...
func (s *memEngineState) rebuildIngressPolicies() {
	...
	if maxPeer == 0 {
		s.ingressExists = s.ingressExists[:0]
	} else {
		need := int(maxPeer) + 1
		if cap(s.ingressExists) < need {
			s.ingressExists = make([]bool, need)
		} else {
			s.ingressExists = s.ingressExists[:need]
	...
	for routeID, route := range s.routes {
		...
		if int(peer) < len(s.ingressExists) {
			s.ingressExists[peer] = true
		}
	}
}

func (s *memEngineState) hasIngressPeer(peer PeerID) bool {
	return int(peer) < len(s.ingressExists) && s.ingressExists[peer]
}
