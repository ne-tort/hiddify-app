...
# experiments/router/bench -> experiments/router/hiddify-sing-box
$scriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$singBoxRoot = Resolve-Path (Join-Path $scriptDir "../hiddify-sing-box")
if ([string]::IsNullOrWhiteSpace($RuntimeDir)) {
  $RuntimeDir = Join-Path $scriptDir "runtime"
}
New-Item -ItemType Directory -Path $RuntimeDir -Force | Out-Null

$benchCmd = 'go test -run "^$" -bench "Benchmark(MemEngineHandleIngress|L3RouterEndToEndSyntheticTransport|SyntheticTransportOnly)" -benchmem -count {0} ./common/l3router' -f $Count
