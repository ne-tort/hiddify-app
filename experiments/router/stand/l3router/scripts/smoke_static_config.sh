...
peers="$(jq '.endpoints[] | select(.type=="l3router") | .peers | length' "$server")"
if [[ "${peers}" -lt 2 ]]; then
  echo "expected >= 2 l3router peers, got ${peers}" >&2
  exit 1
fi
...
echo "[smoke_static_config] ok: server peers=${peers}, overlay=${overlay}"
